	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			if (fd == null) {
				throw new Exception("No s'ha definit la propietat de la ubicació dels fitxers ("
						+ "org.fundaciobit.instanciagenerica.filesdirectory)");
			}
			if (!fd.exists()) {
		...
		try {
			...
			String[] basenames = { "missatges", // /WEB-INF/classes/
					"logicmissatges", "genapp", "instanciagenerica_genapp" };
			...
			if (plugins == null || plugins.size() == 0) {
				...
				for (Class<? extends IExportDataPlugin> class1 : plugins) {
					...
					if (edp == null) {
						...
						DataExporterManager.addDataExporter(new InstanciaGenericaDataExporter(edp));
					}
				}
			}
		...
		try {
			log.info("InstanciaGenerica Version: " + ver);
		} catch (Throwable e) {
			log.info("InstanciaGenerica Version: " + ver);
		}
