...
/**
 * Controller per gestionar un InstanciaGenerica ========= FITXER AUTOGENERAT -
 * NO MODIFICAR !!!!!
 * 
 * @author GenApp
 */
...
public class InstanciaGenericaController extends
		org.fundaciobit.instanciagenerica.back.controller.InstanciaGenericaFilesBaseController<InstanciaGenerica, java.lang.Long, InstanciaGenericaForm>
	...
	public InstanciaGenericaFilterForm getInstanciaGenericaFilterForm(Integer pagina, ModelAndView mav,
		...
		instanciaGenericaFilterForm = (InstanciaGenericaFilterForm) request.getSession()
	...
	public ModelAndView llistatPaginat(HttpServletRequest request, HttpServletResponse response,
			@PathVariable Integer pagina, @ModelAttribute InstanciaGenericaFilterForm filterForm) throws I18NException {
		if (!isActiveList()) {
			response.setStatus(HttpServletResponse.SC_NOT_FOUND);
			return null;
		}
	...
	protected List<InstanciaGenerica> llistat(ModelAndView mav, HttpServletRequest request,
		...
		List<InstanciaGenerica> instanciaGenerica = processarLlistat(instanciaGenericaEjb, filterForm, pagina,
				getAdditionalCondition(request), mav);

		mav.addObject("instanciaGenericaItems", instanciaGenerica);

		mav.addObject("instanciaGenericaFilterForm", filterForm);

		fillReferencesForList(filterForm, request, mav, instanciaGenerica,
				(List<GroupByItem>) mav.getModel().get("groupby_items"));
	...
	public Map<Field<?>, GroupByItem> fillReferencesForList(InstanciaGenericaFilterForm filterForm,
			HttpServletRequest request, ModelAndView mav, List<InstanciaGenerica> list, List<GroupByItem> groupItems)
			throws I18NException {
	...
	public void exportList(@PathVariable("dataExporterID") String dataExporterID, HttpServletRequest request,
			HttpServletResponse response, InstanciaGenericaFilterForm filterForm) throws Exception, I18NException {
	...
	public ModelAndView crearInstanciaGenericaGet(HttpServletRequest request, HttpServletResponse response)
			throws I18NException {
		...
		if (instanciaGenericaForm.getInstanciaGenerica().getTitol() == null) {
			...
			for (org.fundaciobit.instanciagenerica.model.entity.Idioma idioma : instanciaGenericaForm
					.getIdiomesTraduccio()) {
				trad.addTraduccio(idioma.getIdiomaID(),
						new org.fundaciobit.instanciagenerica.persistence.TraduccioMapJPA());
			}
	...
	public void fillReferencesForForm(InstanciaGenericaForm instanciaGenericaForm, HttpServletRequest request,
			ModelAndView mav) throws I18NException {
		// Comprovam si ja esta definida la llista
		if (instanciaGenericaForm.getListOfValuesForSolicitantTipusAdminID() == null) {
			List<StringKeyValue> _listSKV = getReferenceListForSolicitantTipusAdminID(request, mav,
					instanciaGenericaForm, null);
		...
		if (instanciaGenericaForm.getListOfValuesForSolicitantPersonaFisica() == null) {
			List<StringKeyValue> _listSKV = getReferenceListForSolicitantPersonaFisica(request, mav,
					instanciaGenericaForm, null);
	...
	public List<org.fundaciobit.instanciagenerica.model.entity.Idioma> getIdiomesSuportats() throws I18NException {
		List<org.fundaciobit.instanciagenerica.model.entity.Idioma> idiomes = idiomaEjb
				.select(org.fundaciobit.instanciagenerica.model.fields.IdiomaFields.SUPORTAT.equal(true));
		return idiomes;
	}
	...
	protected ModelAndView editAndViewInstanciaGenericaGet(
			@PathVariable("instanciaGenericaID") java.lang.Long instanciaGenericaID, HttpServletRequest request,
		...
		if (instanciaGenerica == null) {
			...
			ModelAndView mav = new ModelAndView(getTileForm());
			InstanciaGenericaForm instanciaGenericaForm = getInstanciaGenericaForm(instanciaGenerica, __isView, request,
					mav);
			instanciaGenericaForm.setView(__isView);
			if (__isView) {
	...
	public ModelAndView editarInstanciaGenericaGet(
			@PathVariable("instanciaGenericaID") java.lang.Long instanciaGenericaID, HttpServletRequest request,
			HttpServletResponse response) throws I18NException {
		return editAndViewInstanciaGenericaGet(instanciaGenericaID, request, response, false);
	}
	...
	public String editarInstanciaGenericaPost(@ModelAttribute InstanciaGenericaForm instanciaGenericaForm,
		...
		try {
			...
			String msg = createMessageError(request, "error.modification", instanciaGenerica.getInstanciaGenericaID(),
					__e);
			log.error(msg, __e);
	...
	public String[] getArgumentsMissatge(Object __instanciaGenericaID, Throwable e) {
		...
		if (instanciaGenericaID == null) {
			return new String[] { I18NUtils.tradueix(getEntityNameCode()), getPrimaryKeyColumnsTranslated(), null,
					exceptionMsg };
		} else {
			return new String[] { I18NUtils.tradueix(getEntityNameCode()), getPrimaryKeyColumnsTranslated(),
					String.valueOf(instanciaGenericaID), exceptionMsg };
		}
	}
	...
	public List<StringKeyValue> getReferenceListForSolicitantTipusAdminID(HttpServletRequest request, ModelAndView mav,
			InstanciaGenericaForm instanciaGenericaForm, Where where) throws I18NException {
	...
	public List<StringKeyValue> getReferenceListForSolicitantTipusAdminID(HttpServletRequest request, ModelAndView mav,
			InstanciaGenericaFilterForm instanciaGenericaFilterForm, List<InstanciaGenerica> list,
	...
	public List<StringKeyValue> getReferenceListForTitolID(HttpServletRequest request, ModelAndView mav,
			InstanciaGenericaFilterForm instanciaGenericaFilterForm, List<InstanciaGenerica> list,
			Map<Field<?>, GroupByItem> _groupByItemsMap, Where where) throws I18NException {
	...
	public List<StringKeyValue> getReferenceListForTitolID(HttpServletRequest request, ModelAndView mav, Where where)
			throws I18NException {
		return traduccioRefList.getReferenceList(TraduccioFields.TRADUCCIOID, where);
	}

	public List<StringKeyValue> getReferenceListForIdiomaID(HttpServletRequest request, ModelAndView mav,
			InstanciaGenericaForm instanciaGenericaForm, Where where) throws I18NException {
	...
	public void preValidate(HttpServletRequest request, InstanciaGenericaForm instanciaGenericaForm,
	...
	public void preList(HttpServletRequest request, ModelAndView mav, InstanciaGenericaFilterForm filterForm)
			throws I18NException {
	}
	...
	public String getRedirectWhenModified(HttpServletRequest request, InstanciaGenericaForm instanciaGenericaForm,
			Throwable __e) {
		if (__e == null) {
